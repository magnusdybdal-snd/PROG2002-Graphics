...
uniform vec3 u_ambientStrength = vec3(1.0);
...
void main()
{
   ...
   if (u_TextureEnabled == 0){
        fragColor = vec4(u_Color, 1.0) * vec4(u_ambientStrength,1.0);
    } else {
        fragColor = mix(vec4(u_Color, 1.0), textureColor, 0.4) * vec4(u_ambientStrength,1.0);
    }
}
)";
